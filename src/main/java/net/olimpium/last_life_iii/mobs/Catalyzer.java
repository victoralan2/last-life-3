...
public class Catalyzer implements Listener {
    ...
    public void NegateArrowDMG(EntityDamageByEntityEvent e){
        if(e.getEntity().getType().equals(EntityType.PILLAGER)&&e.getEntity().getCustomName().contains(ChatColor.DARK_PURPLE + "Catalyzer")){
                ...
                for (UUID uuid : list) {
                    ...
                    if (catalyzer != null) {
                        ...
                        for (double angle = 0; angle <= 2*Math.PI; angle = angle + NumberOfParticles){
                            double x = CatalyzerRadius*Math.cos(angle);
                            double z = CatalyzerRadius*Math.sin(angle);
            ...
            try {
                ...
                for (UUID uuid : list) {
                    ...
                    if (catalyzer != null) {
                        ...
                        List<Entity> entities = catalyzer.getNearbyEntities(CatalyzerRadius, CatalyzerRadius, CatalyzerRadius);
